...
grep -viE 'smiles|inchi|token|key|kaggle|nicholasooo|dataset url|license|/input/' "$ROOT/script.log" | grep -iE 'done|rows|count|min|sec|s/|shard|part|error|traceback' | tail -25 | cut -c1-160
