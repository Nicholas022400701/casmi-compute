...
if [ ! -f wh/.done ]; then mkdir -p wh && cp "$IN"/casmi-m2-fwdsim-wheels/*.whl wh/ && (cd wh && for f in *2.6.0cpu*.whl; do [ -f "$f" ] && mv "$f" "${f/2.6.0cpu/2.6.0+cpu}"; done; for f in *pt26cpu*.whl; do [ -f "$f" ] && mv "$f" "${f/2.1.2pt26cpu/2.1.2+pt26cpu}"; done; true) && touch wh/.done; fi
...
CPU=$(grep -m1 'model name' /proc/cpuinfo | cut -d: -f2 | sed 's/^ //'); log "setup done in $(( $(date +%s) - T_BOOT ))s: $(nproc) cpus ($CPU); torch $(python -c 'import torch;print(torch.__version__)'); src $(head -1 src/COMMIT.txt | cut -d' ' -f1)"
cat > summ.py <<'PY'
...
runOne() {  # $1 shard
  ...
  log "shard $s start"; PYTHONPATH=src python -m "$MODULE" $ARGS --out "$O" --gen "$CK/$GEN" --inten "$CK/$INTEN" --device cpu --threads "$THREADS" --batch "$BATCH" --chunk "$CHUNK" --maxNodes "$MAXNODES" --sparseK "$SPARSEK" --shard "$s/$N" > "$O/run.log" 2>&1; local RC=$?
  python summ.py "$O" "$s" "$N" "$RC" $(( $(date +%s) - T0 )) "${MODULE##*.}" "$JOB" "$KID" | tail -1; log "shard $s done rc=$RC $(( $(date +%s) - T0 ))s"
