...
#   [JOBS_MD5 comma list file=md5 — every listed input is verified after download; mismatch -> exit 1 (D78)]  [ARGS module args]  [MODULE casmi.fwdsim.runShard]
...
for kv in ${JOBS_MD5//,/ }; do f=${kv%%=*}; want=${kv#*=}; [ -f "jobs/$f" ] || { log "md5 pin for unknown input $f"; exit 1; }; have=$(md5sum "jobs/$f" | cut -d' ' -f1); [ "$have" = "$want" ] || { log "input md5 mismatch $f: have $have want $want -> refusing"; exit 1; }; log "md5 ok $f"; done
...
# ---- run ----
T0=$(date +%s)
PYTHONPATH=src python -m "$MODULE" $ARGS --out "$OUT" --gen "$CK/$GEN" --inten "$CK/$INTEN" --device cpu --threads "$THREADS" --batch "$BATCH" --chunk "$CHUNK" --maxNodes "$MAXNODES" --sparseK "$SPARSEK" --shard "$SHARD/$N" > "log/ice_$S.log" 2>&1 &
...
wait $PID; RC=$?; WALL=$(( $(date +%s) - T0 )); log "module rc=$RC wall ${WALL}s"; grep -E '^shard |predictions|s/job' "log/ice_$S.log" | grep -viE 'smiles|inchi' | tail -3 | cut -c1-200
...
for i in $(seq 1 20); do sleep 30; kaggle datasets files "$DS" 2>/dev/null | grep -q "_${S}_pool_summary.json" && { log "dataset ready: $DS"; echo "DONE rc=$RC"; exit $RC; }; done
log "dataset not verified yet: $DS"; echo "DONE (upload unverified) rc=$RC"; exit $RC
